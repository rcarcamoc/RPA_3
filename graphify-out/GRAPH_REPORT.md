...
## Corpus Check
- 207 files · ~173,450 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2547 nodes · 4588 edges · 162 communities (146 shown, 16 thin omitted)
...
Cohesion: 0.06
Nodes (31): Guarda grabación a JSON., ModuleGenerator, Path, Generador de módulos., Genera módulo independiente sin dependencias externas., Genera módulo independiente., QuickScriptGenerator, Generador de scripts rápidos. (+23 more)
...
Cohesion: 0.18
Nodes (7): QTableWidgetItem, LLMPanel, QWidget, Panel de mantenimiento de modelos LLM., Consulta ris.log_llm_ranking para obtener estadísticas de los modelos (es_primer, Lee BASE_LLM_MODELS de llm_config.py e inserta filas en la tabla con estadística, Llamar desde main() para validar sin interacción del usuario.

### Community 8 - "OCREngine"
Cohesion: 0.11
Nodes (16): MockAutomation, Mueve, resalta con círculo rojo y hace clic sostenido., VisualFeedbackLocal, TestCliceAlternativos, Action, ActionType, Enum, Dataclass Action - Inmutable. (+8 more)
...
Cohesion: 0.07
Nodes (23): Test 1: Inicializar OCR Engine, Test 1: Inicializar OCR Engine, Test 3: Code Generator, Test 4: Real Extraction (Dummy Image), test_ocr_code_generation(), test_ocr_extraction(), test_ocr_initialization(), test_ocr_matcher() (+15 more)
...
Cohesion: 0.16
Nodes (8): Action, Doble Click con fallback., Sostiene el clic durante un tiempo (clic largo)., Type con selector o foco., Ejecuta combinación de teclas., Establece el contenido del portapapeles., Mueve el puntero a la posición especificada., Ejecuta una acción según su tipo.
...
Cohesion: 0.18
Nodes (8): QObject, AutoReplaceSignals, AutoReplaceWorker, QThread, Worker para validar el estado básico (HTTP 200) de una lista de modelos., Worker que descarga los populares semanales de OpenRouter y reemplaza los caídos, ValidationSignals, ValidationWorker
...
Cohesion: 0.05
Nodes (25): Captura typing y actualiza modificadores., Actualiza estado de modificadores al soltar., Detecta si es una combinación de teclas importante., Guarda una combinación de teclas., Captura acciones con selectores inteligentes., Obtiene el contenido del portapapeles., Guarda acción de typing usando contexto del último click., Guarda acción de tecla especial. (+17 more)
...
Cohesion: 0.11
Nodes (10): QWidget, Carga lista de grabaciones desde el directorio principal recursivamente., Panel para reproducir., Elimina la grabación seleccionada físicamente., ReplayPanel, OCRInitWorker, QThread, Worker para inicializar OCR en segundo plano. (+2 more)
...
Cohesion: 0.06
Nodes (23): main(), Test script for the new dashboard panel, OperacionesPanel, QWidget, Muestra u oculta campos según el modo de loop seleccionado., Chequea si el servicio de Telegram está ejecutando un flujo mediante el archivo, Detiene la ejecución actual del worker local o del servicio en background., Panel personalizado con botones de operación solicitados. (+15 more)
...
Cohesion: 0.16
Nodes (10): ConsoleNoiseFilter, main(), MainWindow, QMainWindow, Ejecuta la validacion de manera asincrona (no bloqueante) en segundo plano., run_background_llm_validation(), cleanup_old_logs(), PeriodicCleanup (+2 more)
...
Cohesion: 0.23
Nodes (7): enviar_alerta_todos(), KeyUpdateWindow, main(), Verifica si la API Key es válida haciendo una consulta a OpenRouter., Actualiza la clave en el archivo .env., test_api_key(), update_env_key()
...
Cohesion: 0.15
Nodes (20): _consultar_registro(), _formatear_mensaje(), _get_telegram(), handle_error_and_exit(), _marcar_error(), Genera el texto del mensaje Telegram con formato HTML., Punto de entrada único para errores críticos en el workflow PACS.      Pasos:, Importa las funciones de Telegram de manera robusta. (+12 more)
...
Cohesion: 0.12
Nodes (18): RecordingPlayer - Reproduce grabación., Reproduce grabación con validación., Reproduce grabación con validación., Conecta a ventana objetivo., Ejecuta todas las acciones., RecordingPlayer, cmd_generate_module(), cmd_generate_script() (+10 more)
...
- **Why does `VisualFeedback` connect `RPA Visual Feedback` to `OCR Diagnostic Search`, `Doctor Database Search`, `OCREngine`, `OCR Action Library`, `Doctor Database Connector`, `OCR Pattern Matching`, `Immutable Action Executor`, `Clipboard Text Automation`, `Word Toolbar Automation`, `RPA Input Recorder`, `Web Automation Scripting`, `Python Script Generator`, `Exam Search Automation`, `Web Automation Controller`, `Dashboard Analytics Panel`, `Workflow Serialization`, `OCR Status Updater`, `Patient Search Automation`, `Visual OCR Verification`, `Visual Similarity Verification`, `Click and Sort Automation`, `cierra_visor_documentos_v2.py`?**
  _High betweenness centrality (0.112) - this node is a cross-community bridge._
- **Why does `ActionExecutor` connect `OCREngine` to `PACS Automation Workflow`, `RIS Automation Workflow`, `Immutable Action Executor`, `Automated Workflow Execution`, `RPA Visual Feedback`, `Clipboard Text Automation`, `OCR Status Updater`, `Patient Search Automation`, `Click and Sort Automation`, `PACS User Login`, `Recording Replay Panel`, `VPN Connectivity Monitor`, `Pathology Automation Script`, `Mock Automation Feedback`, `Generic Recording Automation`, `Non-Critical Pathology Automation`, `Report Recording Automation`, `Pending Task Automation`, `Windows Element Selector`, `Recording Player Setup`, `RIS Window Management`?**
  _High betweenness centrality (0.083) - this node is a cross-community bridge._
- **Why does `MainWindow` connect `Main GUI Application` to `Web Record Panel`, `Web Replay System`, `Workflow Command Pattern`, `LLM UI Panel`, `Operations Control Panel`, `Script Debugging Panel`, `OCR Configuration Panel`, `Combobox Click Test`?**
